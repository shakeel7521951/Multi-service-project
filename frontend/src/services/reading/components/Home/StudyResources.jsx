...
const StudyResources = () => {
  ...
  return (
    <section
      className="py-20 bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden text-white"
      id="resources"
    >
      {/* Animated background */}
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full font-semibold text-sm md:text-base transition-all duration-300 border-2 backdrop-blur-sm shadow-xl ${
                activeCategory === category.id
                  ? "bg-purple-600/70 text-white border-purple-400 shadow-purple-500/50 scale-105"
          ...
          {filteredResources.length > 0 ? (
            filteredResources.map((item, i) => (
              <a
                ...
                className="group relative block bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 transition-all duration-500 hover:border-purple-400/50 hover:shadow-2xl hover:shadow-purple-500/30 transform hover:-translate-y-1 hover:scale-[1.01] overflow-hidden"
              >
                <div
